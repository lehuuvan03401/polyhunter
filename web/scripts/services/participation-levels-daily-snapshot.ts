...
async function main(): Promise<void> {
    ...
    const date = normalizeDateInput(
        process.env.PARTICIPATION_LEVELS_SNAPSHOT_DATE,
        'PARTICIPATION_LEVELS_SNAPSHOT_DATE'
    );
    const dryRun = parseBool(process.env.PARTICIPATION_LEVELS_DRY_RUN, false);
    const walletAddresses = parseWalletList(process.env.PARTICIPATION_LEVELS_WALLETS);
    ...
    const res = await fetch(`${baseUrl}/api/participation/levels`, {
        method: 'POST',
        headers: {
    ...
    if (!res.ok) {
        throw new Error(
            `[participation-levels-daily] failed status=${res.status} body=${JSON.stringify(data)}`
        );
    }

    console.log(
        `[participation-levels-daily] success snapshotDate=${data.snapshotDate || date || 'auto'} dryRun=${data.dryRun ?? dryRun} processed=${data.processed ?? 0} upserted=${data.upserted ?? 0}`
    );
}
