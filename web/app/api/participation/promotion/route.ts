...
function isAdmin(request: NextRequest): boolean {
    ...
    if (process.env.NODE_ENV === 'development' && ADMIN_WALLETS.length === 0) {
        console.warn('[ParticipationPromotion] Admin auth bypassed in development mode');
        return true;
    }
...
export async function GET(request: NextRequest) {
    try {
        ...
        const [progress] = await buildDoubleZonePromotionProgress(prisma, [walletAddress]);

        const latestSnapshot = await prisma.doubleZoneSnapshot.findFirst({
            where: { walletAddress },
        ...
        console.error('[ParticipationPromotion] GET failed:', error);
        return NextResponse.json({ error: 'Failed to fetch promotion progress' }, { status: 500 });
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        const progressRows = await buildDoubleZonePromotionProgress(prisma, wallets);
        ...
        for (const row of progressRows) {
            ...
            upsertOps.push(
                prisma.doubleZoneSnapshot.upsert({
                    where: {
                    ...
                    update: {
                        leftLegWallet: row.leftLegWallet,
                        rightLegWallet: row.rightLegWallet,
                        leftNetDepositUsd: row.leftNetDepositUsd,
                        rightNetDepositUsd: row.rightNetDepositUsd,
                        weakZoneNetDepositUsd: row.weakZoneNetDepositUsd,
                        strongZoneNetDepositUsd: row.strongZoneNetDepositUsd,
                        promotionLevel: row.promotionLevel,
                        nextLevel: row.nextLevel,
                        nextLevelGapUsd: row.nextLevelGapUsd,
                    },
                    create: {
                        accountId,
                        walletAddress: row.walletAddress,
        ...
        console.error('[ParticipationPromotion] POST failed:', error);
        return NextResponse.json({ error: 'Failed to run promotion snapshot' }, { status: 500 });
    }
}
