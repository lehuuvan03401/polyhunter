...
- [complete] 阶段 3：文档/PR 验收项同步与阶段提交
...
- [x] Phase 2：业务逻辑修复（#1, #4, #7, #11, #12, #13）
- [x] Phase 3：监控与安全（#5, #8, #9）
- [x] Phase 4：防滥用（#17）
...
- [x] 新增管理员取消认购 API（#4）
- [x] 交易历史增加游标分页（#7）
- [x] 清仓任务增加最大重试限制（#11 — 已实现 MAX_ATTEMPTS=20）
- [x] NAV 刷新价格异常时跳过更新（#12）
- [x] Worker 增加会员过期检查步骤（#13）
...
- [x] 账户最低年龄检查：MANAGED_TRIAL_MIN_ACCOUNT_AGE_DAYS（默认 1天）（#17）
- [x] 返回 trialDeniedReason 辅助可观测性
- [x] 更新并增加 5 个测试用例（5/5 通过）
...
- [x] Phase C：资金退改自动化（M8）
  ...
  - [x] 代码实现：构建 `RefundExecutor` 自动脚本，联动预留池拨付。
