...
### 2026-03-04 新增发现：全球合伙人制度剩余高优先级风险
- 排名构建与淘汰候选排序使用了不同 tie-break 规则：排名使用 `scoreActiveManagedUsd`，淘汰候选排序未使用，存在“展示排名与淘汰结果不一致”争议风险。
- `POST /api/partners/seats` 当前允许请求方传入 `seatFeeUsd` 覆盖 `refillPriceUsd`，在缺少外部支付核验时会破坏补位定价约束与退款基准。
- 管理接口鉴权仍以 `x-admin-wallet` 头为主，缺少签名校验，抗伪造能力弱于用户侧钱包签名接口。
- 淘汰评分读取“最新快照”而非严格按 `monthKey` 边界统计，存在跨月数据污染当月淘汰结果的风险。

### 2026-03-04 收口结果（P0/P1）
- 排名与淘汰候选排序已统一为同一套比较器，淘汰名单由排名逆序直接导出，消除显示/执行不一致风险。
